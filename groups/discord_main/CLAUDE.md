...
- **업무일지 작성 완료 시 전문 첨부** — "업무일지 작성 완료"를 보고할 때는 반드시 작성된 업무일지 전문을 메시지에 함께 포함해서 전달한다
...
### ✅ 업무지시 허용 사용자 (파일 수정 제외)

학생회 전원(7명)은 아래 업무지시 가능. CRON 등록/삭제는 파일 수정 권한자(성호, 요나새)만 가능.
...
| `537616840898248705` | 나린 (xonorous) |
...
3. **화이트리스트 조회**:
   ...
   - ❌ **목록에 없음** → 거절 메시지
4. **파일 수정 권한 없는 학생회 멤버**: 죨디, 호녈, 초코슈, 운명교향곡, 나린이 편집 요청하면 거절. 업무지시(메시지 게시, DB 조회, 기숙사 변경, 다이어리 생성)는 수행.
...
"완료했어요"라고 보고한 직후 스스로 재확인해서 누락이나 실수를 발견해도, **즉시 이어서 추가 수정 + 새 deploy.flag를 작성하지 말 것**. 반드시 먼저 관리자에게 보고하고 다음 지시를 기다린다.

- ✅ "방금 작업에서 X가 빠진 것 같아요. 추가로 이전할까요?" (묻고 대기)
- ❌ 바로 Edit 도구 + 새 deploy.flag 작성 (자동 실행)

**이유**: deploy는 NanoClaw 호스트 재시작을 트리거하고, 그 순간 **현재 컨테이너는 orphaned으로 강제 종료**된다. 지금 진행 중인 세션 — 특히 관리자의 그 직전 질문을 처리 중이었다면 — 이 통째로 유실된다. 이미 DB에 `processed` 마킹된 메시지는 재시작 후 recovery에서도 제외되므로 **관리자는 답변을 영영 못 받는다**. 실제로 2026-04-23 23:50 사고에서 성호의 "현재 이렇게 되어있는데?" 메시지가 이 방식으로 유실됐다.

**올바른 2-turn 패턴**:

\`\`\`
Turn 1 (작업 실행):
  → Edit + deploy.flag → "완료했어요 ✅" 보고 → 재시작

Turn 2 (관리자의 다음 메시지가 온 뒤):
  → ls로 확인했더니 누락 발견 → "X가 빠져있네요. 추가로 이전할까요?" (묻기만)

Turn 3 (관리자의 "네"를 받은 뒤):
  → 그제서야 Edit + deploy.flag
\`\`\`

**단일 turn 안에 "작업 → 보고 → 검증 → 재작업 → 재deploy"를 이어서 하면 안 된다.** 검증 결과가 수정을 유발할 수 있을 때는 *반드시* 그 시점에서 멈추고 관리자 확인을 받는다.
