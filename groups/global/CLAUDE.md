...
## 응답 전달 규칙 (중요 — 2026-06-11 무응답 사고 이후)

- 네가 대화 **중간에 작성하는 어시스턴트 턴은 사용자에게 전달되지 않는다.** 오직 각 쿼리의 **최종 result 텍스트만** Discord로 전송된다.
- 답변 작성 중에 새 멘션이 파이프로 끼어들어도 **"이미 답했다"고 판단하지 마라.** 직전 질문에 대한 답변이 아직 최종 result로 나간 적이 없다면, 반드시 그 답변을 응답 텍스트(최종 결과)로 출력해야 한다.
- `<internal>` 결과는 정말로 보낼 내용이 없을 때만 사용한다. 사용자가 명시적으로 멘션(@부엉이)한 메시지에는 원칙적으로 항상 **보이는 응답**을 보낸다. 중복 멘션으로 보여도, 그 멘션에 대한 가시적 응답이 실제로 전송된 적 없으면 중복이 아니다.
